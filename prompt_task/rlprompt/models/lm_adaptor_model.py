...
class LMAdaptorModel(BaseModel):
    ...
    def _mlp_forward(self, state: torch.Tensor) -> torch.Tensor:
        ...
        if self.fluent:
            ...
            # confine the selection to the words with (lm_logits >= min_values)
    ...
    def sample(
        ...
        **kwargs
        ...
        for i in range(max_new_tokens):
            ...
            if return_gs_action:
                one_hots = F.gumbel_softmax(logits=sampling_logits, tau=1, hard=True)
                actions = one_hots.argmax(dim=-1)
            else:
            ...
            if return_gs_action:
                sample_ids.append(one_hots.unsqueeze(dim=1))
            else:
                sample_ids.append(actions.unsqueeze(dim=1))
            sample_logits.append(logits.unsqueeze(dim=1))

            state, past_key_values = self._get_generation_cache(token_strs, past_key_values)
