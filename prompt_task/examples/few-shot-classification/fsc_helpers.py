...
class PromptedClassificationDataset(Dataset):
    def __init__(
        ...
        self.source_texts = source_texts        # source_texts: text to classification
        self.class_labels = class_labels        # class_labels: ground-truth class labels
...
def get_prompts_for_eval(output_folder: str):
    store_loc = os.path.join(output_folder, "outputs/eval")
    prompt_files = [x for x in os.listdir(store_loc) if ".json" in x]
    print(f"# of candidate prompts: {len(prompt_files)}")
    prompts_scores = {"prompts": [], "scores": []}
    for pfile in prompt_files:
        file_loc = os.path.join(store_loc, pfile)
        with open(file_loc) as f:
            temp = json.loads(f.read())
            prompts_scores["prompts"].append("".join(temp['output_tokens'][0]).replace("Ġ", " "))
            prompts_scores["scores"].append(temp['scores'][0])

    save_loc = os.path.join(output_folder, "prompts_scores.csv")

    assert os.path.isfile(save_loc)
    stored_prompts = pd.read_csv(save_loc)
    print("\nStored Prompts")
    print(stored_prompts.head())

    df = pd.DataFrame.from_dict(prompts_scores)
    df = pd.concat([df, stored_prompts], axis=0)
    print(f"Shape of df before drop_duplicates: {df.shape}")
    df = df.drop_duplicates(subset=["prompts"], keep='first')
    print(f"Shape of df after drop_duplicates: {df.shape}")

    df = df.sort_values(by="scores", ascending=False)
    df = df.head(16)
    print("\nStored Prompts with last K outputs")
    print(df.head())
